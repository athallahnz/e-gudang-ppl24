...
if (isset($_POST['submit'])) {
    ...
    // Menggunakan fungsi decrypt untuk mendapatkan ID penugasan yang di-edit
    ...
    // Update penugasan_aset dengan prepared statement untuk mencegah SQL Injection
    $stmt = $connect->prepare("UPDATE penugasan_aset SET nama=?, deskripsi=?, last_edited=now(), edited_by=? WHERE id=?");
    ...
    if ($update) {
        ...
        $stmt_log = $connect->prepare("INSERT INTO userlog (fk_id_user, ip_address, agent, waktu, status_log, aplikasi, komponen, event_context) 
                                       VALUES (?, ?, ?, now(), ?, 'sim-barang', 'Master Penugasan', ?)");
        $status_log = "User dengan ID ($id_user) mengedit penugasan dengan ID ($id)";
        ...
        if ($logs) {
            ...
            header('Location: menu-edit-penugasan?id=' . encrypt_decrypt2('encrypt', $id));
        } else {
