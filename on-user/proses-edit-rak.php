...
if (isset($_POST['submit'])) {
    ...
    $stmt = $connect->prepare("UPDATE rak_aset SET nama=?, deskripsi=?, last_edited=now(), edited_by=? WHERE id=?");
    ...
    if ($update) {
        ...
        $stmt_log = $connect->prepare("INSERT INTO userlog (fk_id_user, ip_address, agent, waktu, status_log, aplikasi, komponen, event_context) 
                                       VALUES (?, ?, ?, now(), ?, 'sim-barang', 'Master Rak', ?)");
        $status_log = "User dengan ID ($id_user) mengedit rak dengan ID ($id)";
        ...
        if ($logs) {
            ...
            header('Location: menu-edit-rak?id=' . encrypt_decrypt2('encrypt', $id));
        } else {
