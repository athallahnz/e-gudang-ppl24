...
if (!$token || $token !== $_SESSION['token']) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $penugasan = htmlspecialchars($_POST['penugasan']);
        ...
        $stmt = $connect->prepare("INSERT INTO penugasan_aset (nama, deskripsi, created_by, last_created) VALUES (?, ?, ?, now())");
        $stmt->bind_param("ssi", $penugasan, $deskripsi, $id_user);
        $simpan = $stmt->execute();
        ...
        if ($simpan) {
            ...
            $status_log = "User dengan ID ($id_user) menambahkan penugasan baru dengan ID ($last_id)";
