...
if (!$token || $token !== $_SESSION['token']) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $stmt = $connect->prepare("INSERT INTO rak_aset (nama, deskripsi, created_by, last_created) VALUES (?, ?, ?, now())");
        $stmt->bind_param("ssi", $rak, $deskripsi, $id_user);
        $simpan = $stmt->execute();
        ...
        if ($simpan) {
            ...
            $status_log = "User dengan ID ($id_user) menambahkan rak baru dengan ID ($last_id)";
