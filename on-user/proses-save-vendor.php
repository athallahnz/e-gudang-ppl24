...
if (!$token || $token !== $_SESSION['token']) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $stmt = $connect->prepare("INSERT INTO vendor_aset (nama, deskripsi, created_by, last_created) VALUES (?, ?, ?, now())");
        $stmt->bind_param("ssi", $vendor, $deskripsi, $id_user);
        $simpan = $stmt->execute();
        ...
        if ($simpan) {
            ...
            $status_log = "User dengan ID ($id_user) menambahkan vendor baru dengan ID ($last_id)";
